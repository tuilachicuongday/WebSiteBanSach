...
namespace BanSach.Services
{
    public class OrderService
    {
        ...
        public List<Order> SearchOrder(string userID, string status, int pageNo, int pageSize)
        {
        ...
        public Order GetOrderByID(int ID)
        {
            using (var context = new BanSachContext())
            {
                return context.Orders.Where(x => x.ID == ID).Include(x => x.OrderItems).Include("OrderItems.Book").FirstOrDefault();
            }
        }
